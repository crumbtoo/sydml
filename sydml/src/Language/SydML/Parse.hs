...
import Sydc.Error
import Text.Megaparsec (manyTill, ParseErrorBundle(..), parse, eof)
...
type instance Common.PassImports Parse = List Surface.Import

parseSydML :: FilePath -> Text -> Either (List1 SydError) (Module Parse)
parseSydML = decodeMany

--------------------------------------------------------------------------------

-- data Syntax
--   = SyntaxNode !SrcSpan !Text (List Syntax)
--   -- | A node with no children, whose source text we are interested in.
